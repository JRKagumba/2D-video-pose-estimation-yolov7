...
from utils.plots import colors, plot_one_box, output_to_target
...
def detect(opt):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        pred = non_max_suppression(
            pred, opt.conf_thres, opt.iou_thres, classes=opt.classes, agnostic=opt.agnostic_nms, kpt_label=kpt_label)
            ...
            if len(det):
                ...
                for det_index, (*xyxy, conf, cls) in enumerate(reversed(det[:,:6])):
                    ...
                    if save_img or opt.save_crop or view_img:  # Add bbox to image
                        ...
                        plot_one_box(xyxy, im0, label=label, color=colors(c, True), line_thickness=opt.line_thickness, kpt_label=kpt_label, kpts=kpts, steps=3, orig_shape=im0.shape[:2], nobbox= opt.nobbox)
                        if opt.save_crop:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nobbox', action='store_true', help='do not show bounding box aroun person')
